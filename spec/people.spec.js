...
describe('POST /api/people', function() {
  it('should create a person', async function() {
    ...
    const res = await supertest(app)
      ...
      // Check that the response is 201 Created with a JSON body.
      .expect(201)
    ...
    expect(res.get('Location'), 'res.headers.location').to.equal(`${baseUrl}/api/people/${body.id}`);
  });
});
...
describe('GET /api/people', function() {
  ...
  it('should retrieve a list of people', async function() {
    ...
    expect(body[0].directedMovies, 'res.body[0].directedMovies').to.equal(1);
    ...
    expect(body[0], 'res.body[0]').to.have.all.keys('birthDate', 'createdAt', 'directedMovies', 'gender', 'id', 'name');
    ...
    expect(body[1], 'res.body[1]').to.have.all.keys('birthDate', 'createdAt', 'directedMovies', 'gender', 'id', 'name');
